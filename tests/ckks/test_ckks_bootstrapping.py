...
class TestBootstrappingMethods(unittest.TestCase):
    ...
    def run_test_coeff_to_slot(self, m):
        N = len(m)
        plain = self.encoder.encode(m, self.scaling_factor)
        plain_ans1 = mat.scalar_multiply(plain.p.coeffs[:N], 1 / self.params.scaling_factor)
        plain_ans2 = mat.scalar_multiply(plain.p.coeffs[N:], 1 / self.params.scaling_factor)
        ...
        print("expected: " + str(plain_ans1[:10]))
        print("actual: " + str(decrypted_1[:10]))
        check_complex_vector_approx_eq(plain_ans1, decrypted_1, error=0.01)

        print("expected: " + str(plain_ans2[:10]))
        print("actual: " + str(decrypted_2[:10]))
        check_complex_vector_approx_eq(plain_ans2, decrypted_2, error=0.01)

    def run_test_slot_to_coeff(self, m):
        N = self.degree
        plain = self.encoder.encode(m, self.scaling_factor)
        ciph = self.encryptor.encrypt(plain)
        ...
        ciph_ans = self.evaluator.slot_to_coeff(ciph1, ciph2, rot_keys, conj_key, self.encoder)
        decrypted = self.decryptor.decrypt(ciph_ans)
        plain_ans1 = mat.scalar_multiply(decrypted.p.coeffs[:N//2], 1 / decrypted.scaling_factor)
        plain_ans2 = mat.scalar_multiply(decrypted.p.coeffs[N//2:], 1 / decrypted.scaling_factor)

        prim_root = math.e ** (math.pi * 1j / N)
        primitive_roots = [prim_root] * (N // 2)
        for i in range(1, N // 2):
            primitive_roots[i] = primitive_roots[i - 1] ** 5

        mat_0 = [[1] * (N // 2) for _ in range(N // 2)]
        mat_1 = [[1] * (N // 2) for _ in range(N // 2)]
        ...
        new_plain = [plain_1[i] + plain_2[i] for i in range(N // 2)]

        encoded = self.encoder.encode(new_plain, self.scaling_factor)
        plain_check1 = mat.scalar_multiply(encoded.p.coeffs[:N//2], 1 / decrypted.scaling_factor)
        plain_check2 = mat.scalar_multiply(encoded.p.coeffs[N//2:], 1 / decrypted.scaling_factor)
        ...
        check_complex_vector_approx_eq(m, new_plain, error=0.001)

    def run_test_exp(self, m):
        p = Polynomial(self.degree // 2, m)
        plain = self.encoder.encode(m, self.scaling_factor)
        plain_exp = [0] * (self.degree // 2)
        const = 1
        for i in range(self.degree // 2):
            r = 10
            x = m[i] * const / (2**r)
            plain_exp[i] = 1 / 6 * x ** 3 + 1 / 2 * pi ** 2 * x ** 2 + x + 1
            plain_exp[i] += 1 / 120 * x ** 5 + 1 / 24 ** 4 * x ** 4
            plain_exp[i] += 1 / 5040 * pi ** 7 * x ** 7 + 1 / 720 * pi ** 6 * x ** 6
            
            for _ in range(r):
                plain_exp[i] = plain_exp[i] ** 2
        ...
        print("expected: " + str(plain_exp[:10]))
        print("actual: " + str(decrypted_exp[:10]))
        check_complex_vector_approx_eq(plain_exp, decrypted_exp, error=0.1)

    def test_coeff_to_slot_01(self):
        m = sample_random_complex_vector(self.degree // 2)
        self.run_test_coeff_to_slot(m)

    def test_slot_to_coeff_01(self):
        m = sample_random_complex_vector(self.degree // 2)
        self.run_test_slot_to_coeff(m)
...
class TestBootstrapping(unittest.TestCase):
    ...
    def taylor_exp(self, vec, const):
        taylor_exp_vec = [0] * len(vec)
        for i in range(len(vec)):
            x = vec[i] * const / (2 ** self.num_taylor_exp_iterations)
            taylor_exp_vec[i] = 1 / 6 * x ** 3 + 1 / 2 * x ** 2 + x + 1
            taylor_exp_vec[i] += 1 / 120 * x ** 5 + 1 / 24 * x ** 4
            taylor_exp_vec[i] += 1 / 5040 * x ** 7 + 1 / 720 * x ** 6
            
            for _ in range(self.num_taylor_exp_iterations):
    ...
    def run_test_bootstrap_steps(self, message):
        ...
        print(math.sin(2 * pi * plain.p.coeffs[0] / self.ciph_modulus))
        print(2 * pi * (plain.p.coeffs[0] % self.ciph_modulus) / self.ciph_modulus)
        print(math.sin(2 * pi * plain.p.coeffs[0] / self.ciph_modulus) * self.ciph_modulus / 2 / pi)
        ...
        taylor_exp0 = self.taylor_exp(plain_slots0, const)
        taylor_exp1 = self.taylor_exp(plain_slots1, const)
        ...
        check_complex_vector_approx_eq(decoded_neg_exp0, neg_exp0, error=0.001, error_message="EXP FAILED")
        check_complex_vector_approx_eq(decoded_neg_exp1, neg_exp1, error=0.001, error_message="EXP FAILED")
        ...
        check_complex_vector_approx_eq(plain, new_plain, error=0.05, error_message="SLOT TO COEFF FAILED")

        print("-------- ANSWER -------")
        print(new_plain)
        check_complex_vector_approx_eq(message, new_plain, error=0.05, error_message="FINAL CHECK FAILED")
    ...
    def run_test_bootstrap(self, m):
        ...
        ciph, new_ciph = self.evaluator.bootstrap(ciph, rot_keys, conj_key, self.relin_key, self.encoder)    
        new_plain = self.decryptor.decrypt(new_ciph)
        new_plain = self.encoder.decode(new_plain)
        check_complex_vector_approx_eq(m, new_plain, error=0.05)    
